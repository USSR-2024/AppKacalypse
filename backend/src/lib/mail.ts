...
<div style="font-size:18px;font-weight:600;margin-bottom:16px">AppKacalypse</div>
${body}
...
<p style="margin:0;color:#5b6270;font-size:14px">Код действует ${minutes} минут. Никому его не сообщайте.</p>`);
  return send(to, `${code} — код для входа в AppKacalypse`, html, `Код для входа: ${code}\nДействует ${minutes} минут.`);
}
...
Регистрация в AppKacalypse — только по приглашению: попросите ссылку у администратора вашего пространства.</p>`);
  return send(to, 'Запрос входа в AppKacalypse', html, 'Аккаунта с этой почтой нет. Регистрация — только по приглашению от администратора.');
}
