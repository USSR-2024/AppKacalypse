...
## Имя продукта — **appka.space**
Старое `AppKacalypse` осталось только в имени репозитория, бота (`@appKACAlypse_bot`), контейнеров
(`akc-*`) и БД (`akc`) — переименовывать их не нужно, это внутренние идентификаторы. В глазах
пользователя — везде `appka.space`: вкладка, PWA, экран входа, письма, кнопки бота.
⚠️ `id` в `manifest.webmanifest` — **`/today`, не трогать**: по спеке id = «личность» приложения,
его смена = новое приложение для установленных PWA (попросят переставить). `name`/`short_name` менять безопасно.
⚠️ `MAIL_FROM` живёт в `backend/.env` на 158 — правка дефолта в `env.ts` на прод НЕ влияет.
...
- **Публичные ссылки (`/join/<code>`) нужны с og-разметкой**: мессенджер-бот JS не выполняет, теги
  должны быть в `<head>` — значит серверный `generateMetadata`, а клиентский код выносить в
  отдельный компонент. `metadataBase` в корневом layout обязателен, иначе og:image не станет
  абсолютным. Next сам блокирует стриминг метаданных для ботов (Telegram ловится как `TwitterBot`).
  Проверять так: `curl -A "TelegramBot (like TwitterBot)" <url> | grep og:`.
